...
# --- 설정 ---
DATA_PATH = "starbucks_stores/data/starbucks_stores.csv"
...
def main():
    ...
    # theme_state에서 특성 추출 및 변환
    all_theme_names = []
    df['theme_state'].dropna().str.split('@').apply(
        lambda themes: all_theme_names.extend(
            starbucks_service_codes.get(theme, theme) for theme in themes if theme
        )
    )

    feature_summary = pd.Series(all_theme_names).value_counts()
    ...
    if not feature_summary.empty:
        ...
        add_to_report("매장별로 제공되는 서비스와 특징을 분석하여 상위 10개 항목을 표시합니다. '사이렌오더', '현금없는매장', '주차가능' 등의 서비스가 많은 매장에서 제공되는 것을 확인할 수 있습니다.")
        add_to_report(feature_summary_df.head(10).to_markdown(index=False))

        fig, ax = plt.subplots(figsize=(12, 8))
        sns.barplot(data=feature_summary_df.head(10), x='매장 수', y='특징', ax=ax)
